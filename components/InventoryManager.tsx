...
const InventoryManager: React.FC<Props> = ({ products, onAddProduct, onUpdateStock, onDeleteProduct }) => {
  ...
  const isService = newProduct.category === 'Service';
  ...
  const handleAddProduct = () => {
    ...
    onAddProduct({
      ...
      buyingPrice: isService ? 0 : Number(newProduct.buyingPrice),
      sellingPrice: Number(newProduct.sellingPrice),
      stockCount: isService ? 0 : Number(newProduct.stockCount),
      lowStockThreshold: isService ? 0 : Number(newProduct.lowStockThreshold)
    });
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="bg-red-500/5 p-6 rounded-2xl border border-red-500/20 shadow-lg">
          ...
          <p className="text-3xl font-black text-red-400">{products.filter(p => p.category !== 'Service' && p.stockCount <= p.lowStockThreshold).length}</p>
        </div>
            ...
            <tbody className="divide-y divide-slate-700/50">
              {filteredProducts.map(product => (
                <tr key={product.id} className="hover:bg-slate-700/30 transition-colors">
                  ...
                  {/* ✅ Show "Labor Service" instead of stock for services */}
                  <td className="px-6 py-5">
                    {product.category === 'Service' ? (
                      <span className="text-slate-500 italic">Labor Service</span>
                  ...
                  {/* ✅ No stock status badge for services */}
                  <td className="px-6 py-5">
                    {product.category === 'Service' ? (
                      <span className="text-slate-500 text-[10px] font-black bg-slate-800 px-3 py-1 rounded-full border border-slate-700 tracking-widest">
                        SERVICE
                      </span>
                  ...
                  <td className="px-6 py-5 text-right">
                    {/* ✅ Hide edit button for services (no stock to edit) */}
                    {editingId !== product.id && product.category !== 'Service' && (
                      <button 
          ...
          <div className="bg-slate-900 rounded-[3rem] w-full max-w-2xl p-10 border border-slate-800 shadow-3xl">
            ...
            <div className="grid grid-cols-2 gap-6">
              <div className="col-span-2">
                ...
                <input 
                  ...
                  placeholder={isService ? 'e.g., Drum Cleaning, Printer Maintenance' : 'e.g., Riso Ink S-4254 Black'}
              ...
              {/* ✅ Conditional fields: Service vs. Product */}
              {isService ? (
                <>
                  <div className="col-span-2">
                    <label className="block text-[10px] font-black text-slate-500 uppercase tracking-widest mb-3">Service Detail</label>
                    <textarea
                      placeholder="Describe the service (e.g., Printer maintenance, Drum cleaning)"
                      className="w-full px-6 py-4 bg-slate-800 text-white border border-slate-700 rounded-2xl font-bold outline-none focus:ring-4 focus:ring-blue-500/20 h-24 resize-none"
                      onChange={(e) => setNewProduct({...newProduct, name: newProduct.name || e.target.value})}
                    />
                  </div>
                  <div className="col-span-2">
                    <label className="block text-[10px] font-black text-slate-500 uppercase tracking-widest mb-3">Service Price (KES)</label>
                    <input 
                      type="number" 
                      className="w-full px-6 py-4 bg-slate-800 text-white border border-slate-700 rounded-2xl font-bold outline-none focus:ring-4 focus:ring-blue-500/20"
            ...
            <div className="flex gap-4 pt-10 border-t border-slate-800 mt-10">
              <button 
                ...
                className="flex-1 px-6 py-4 border border-slate-700 text-slate-500 rounded-2xl font-black uppercase text-xs tracking-widest hover:bg-slate-800 transition-all"
              >
                Cancel
              </button>
              <button
              ...
              >
                <span>Commit {isService ? 'Service' : 'to Inventory'}</span>
              </button>
            </div>
          </div>
        </div>
